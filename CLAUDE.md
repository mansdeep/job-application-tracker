...
**Known limitation: magic-link email only reaches the Resend account owner in production.** `EMAIL_FROM` is set to `onboarding@resend.dev`, Resend's built-in test sender — no domain is verified on the account (checked directly, `resend.com/domains` shows "No domains yet"). Resend hard-blocks that combination server-side with a 403 (`"You can only send testing emails to your own email address"`) for any recipient other than the account owner — confirmed by calling the Resend API directly with the production key. This means *any other person's* sign-in magic link silently fails to send (surfaces to them as the generic "We couldn't send that link" on `/login`), while the account owner's own sign-in works fine, which is what makes this easy to miss in testing. Real fix: verify a real domain in Resend (Domains → Add domain → add the DNS record it gives you at your registrar → wait for verification) and change `EMAIL_FROM` to an address on that domain, in both `.env` and the Vercel env var. No code change needed once that's done.
