...
`auth.config.ts` (edge-safe: no adapter, no Node-only providers) vs `auth.ts` (full config: Prisma adapter + Credentials + Google providers). `proxy.ts` (Next 16's renamed `middleware.ts`) imports **only** `auth.config.ts` — importing the full `auth.ts` there would pull the Prisma client (which needs the Node `pg` driver) into the Edge runtime and break the build. Session strategy is JWT, not database, specifically because Edge middleware can't hit the database to validate a database session. Every API route calls `requireSession()` from `lib/auth.ts`, which scopes all Prisma queries to `session.user.id` — this is the entire per-user data isolation mechanism, so it must not be skipped in a new route.

**Sign-in: email/password (Credentials) + optional Google.** Replaced the original magic-link (Resend Email provider) design — Resend's account has no verified domain, so magic-link email only ever reached the account owner in practice (see git history / `lib/notify.ts` era for that whole saga), and password auth sidesteps needing outbound email to work at all for the core sign-in path. `User.passwordHash` (nullable — null for Google-only accounts) is checked in `auth.ts`'s `Credentials` provider `authorize()` via `lib/password.ts` (bcryptjs, no native bindings — deliberately, given this project's history of native/ESM packages breaking specifically on Vercel's Node runtime while working fine locally; see the `pdf-parse`/`jsdom` writeups above). Google is added to `providers[]` only when `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` are both set (`auth.ts#googleEnabled`, also read by the login/register pages to conditionally render the "Continue with Google" button) — so an unconfigured deployment doesn't show a button that 500s on click.

**Real gotcha hit while building this: `signIn()` does not auto-redirect-on-error for the Credentials provider the way it did for the old Email provider.** A failed `signIn("credentials", ...)` throws a `CredentialsSignin` `AuthError` that, uncaught, surfaces as a raw 500/error-overlay in dev — it does **not** automatically redirect to `pages.error` the way OAuth/Email provider failures do. Every `signIn()` call in `app/login/page.tsx` and `app/register/page.tsx` must wrap in `try { await signIn(...) } catch (err) { if (err instanceof AuthError) redirect(...); throw err; }` — rethrowing non-`AuthError`s is required because `signIn()`'s own *successful* redirect is itself implemented as a thrown `NEXT_REDIRECT` (not an `AuthError`), so a blanket catch-and-suppress would break the success path too.

**Migration gotcha: pre-existing accounts (created back when magic-link was the only sign-in method) have `passwordHash: null` and cannot sign in with a password until they set one.** This includes the admin's own account. Fixed by the password-reset flow below, which works for setting the *first* password on such an account just as well as resetting a forgotten one — there's no separate "activate your account" flow, `/forgot-password` doubles as it.

**Password reset** (`lib/reset-password.ts`, `app/forgot-password/page.tsx`, `app/reset-password/[token]/page.tsx`): reuses the `VerificationToken` table, which the old magic-link flow used and is otherwise unused now. `/forgot-password` always shows the same "if an account exists…" response regardless of whether the email is registered, to avoid leaking which emails have accounts. Token verification is split into a non-consuming `verifyPasswordResetToken` (safe to call when just rendering the reset form on GET) and an explicit `deletePasswordResetToken` (called only once the new password is actually being set) — collapsing these into one consume-on-read function would burn the token before the user ever sees the form. Sending the reset email only actually succeeds for the Resend account owner right now (see the sandbox-mode note in `lib/notify.ts`'s history/comments) — fine for the admin resetting their own password, not yet useful for other users until a domain is verified in Resend.

### New-user approval gate

New sign-ups aren't usable until an admin approves them: `User.isApproved` defaults to `false`, and `requireSession()` (in `lib/auth.ts`) throws `UnapprovedError` for a signed-in-but-unapproved user — so the approval gate applies everywhere `requireSession()` already does (every API route, plus `board`/`profile` pages), with no extra call site changes needed. Emails in `ADMIN_EMAILS` (comma-separated env var) always pass this check regardless of their DB `isApproved` value — see `lib/admin.ts#isAdminEmail`. That file deliberately has no dependency on `lib/auth.ts` or `auth.ts`: `auth.ts`'s `events.createUser` hook needs `isAdminEmail`/`adminEmails` too (to email admins when a new *Google* sign-up happens — Credentials-based registration instead calls `lib/notify.ts#notifyAdminsOfNewUser` directly from `app/register/page.tsx`'s Server Action, since that flow doesn't go through the adapter's `createUser` event the same way), and `auth.ts` importing from `lib/auth.ts` would be circular (`auth.ts` → `lib/auth.ts` → `auth.ts`, since `lib/auth.ts` imports `auth` from `auth.ts`).
